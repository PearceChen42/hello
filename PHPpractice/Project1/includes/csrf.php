...
function setCsrfTokenAndTime(?string $token, ?int $time):void {
    if ($token == null) {
...
function getCurrentCsrfToken(): string {
    ...
    if (!isset($token,$time) || (time() - $_SESSION['csrf_token_time'] > CSRF_TOKEN_LIFETIME ) )
    {
